...
5. **`src/core.jl`** — the public type system and dispatch:
   ...
   - `JavaObject{T}` — mutable struct wrapping a `JavaRef`, where `T` is a `Symbol` of the fully-qualified Java class name. Constructed via `@jimport`. Finalizer auto-calls `deleteref`. Built-in `J*` aliases (`JObject`, `JClass`, `JString`, etc.) cover the most common Java standard-library types; in 0.10 this set was broadened to 31 aliases spanning collections, boxed primitives, IO types, and a few `java.util` extras.
   ...
   - `jcall`, `jnew`, `jfield`, `jlocalframe`, `jglobal`, `metaclass`, plus the JNI signature builder `signature` / `method_signature`. Each of `jcall`/`jnew`/`jfield` starts with `assertloaded()` and does its JNI work inside a `with_env` block. In addition to the explicit `(ArgTypes...,), args...` form, resolved forms `jcall(receiver, "m", args...)` / `jnew(T, args...)` (added in 0.10) delegate to the overload resolver in `src/overload.jl`.
...
10. **`src/overload.jl`** (Phase 3) — the shared overload resolver used by both the resolved `jcall`/`jnew` forms and `JProxies`. Scores candidate methods by argument type (exact match > subclass > boxing/widening), selects the best match, and throws on ambiguity.
11. **`src/jcall_macro.jl`** — `@jcall` macro that parses Julia call syntax (`@jcall System.getProperty("foo"::JString)::JString`) into the underlying `jcall(...)` form. Also handles `@jimport` multi-import grammars: `@jimport java.util: ArrayList, HashMap` (colon/package syntax) and `@jimport (java.util.ArrayList, java.lang.System)` (tuple syntax), both supporting `=>` renames. See `test/jcall_macro.jl` for the full grammar.
...
- **`Pkg.test` only needs `Pkg` and `Test`** (the only `[targets].test` deps since Taro / DataFrames were removed in the 0.9.0 prep). JProxies' own tests are run separately and only need `Test` + a `Pkg.develop(path="..")` of JavaCall. The non-gating `downstream.yml` workflow covers Taro / JDBC / Spark / BioformatsLoader as a tripwire instead.
